...
class ApplicationController < ActionController::Base
  ...
  def create
    set_object
    if @object.save == true
      flash[:notice] = "New #{controller_name.singularize.sub('_', ' ')} added."
  ...
  def destroy
    ...
    if @object.destroyed? == true
      flash[:notice] = "The
        #{controller_name.singularize.sub('_', ' ')} was deleted!"
    else
      flash[:error] = "An error occured while
        attempting to delete a #{controller_name.singularize.sub('_', ' ')}."
    end
    redirect_to(controller: controller_name.to_s, action: "index")
  end
  ...
  def update
    ...
    if @result == true
      flash[:notice] = "The
      #{controller_name.singularize.sub('_', ' ')} was updated."
      redirect_to(controller: controller_name.to_s, action: "index")
    else
      render action: "edit"
    end
